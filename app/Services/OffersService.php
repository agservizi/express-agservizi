...
final class OffersService
{
     ...
     * }
     */
    public function paginateOffers(int $page, int $perPage, ?string $status = null, ?string $search = null): array
    {
        $page = max($page, 1);
        ...
        if ($searchTerm !== null) {
            $searchCondition = '(
                providers.name LIKE :search_provider
                OR operator_offers.title LIKE :search_title
                OR operator_offers.description LIKE :search_description
            )';
            if (preg_match('/^-?\d+(?:[\.,]\d+)?$/', $searchTerm) === 1) {
                $searchPrice = (float) str_replace(',', '.', $searchTerm);
                $searchCondition = '(' . $searchCondition . ' OR operator_offers.price = :search_price)';
            }
        ...
        $baseFrom = 'FROM operator_offers LEFT JOIN providers ON providers.id = operator_offers.provider_id';
