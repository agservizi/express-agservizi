...
final class SalesController
{
    ...
    public function create(int $userId, array $input): array
    {
        ...
        $customerNote = isset($input['customer_note']) ? trim((string) $input['customer_note']) : null;
        ...
        try {
            ...
            try {
                $saleDetails = $this->salesService->getSaleWithItems($saleId);
                if ($saleDetails !== null) {
                    $emailDispatched = $this->saleNotificationService->sendSaleCompletedEmail($saleDetails);
                }
            } catch (\Throwable $notificationError) {
                // Ignore email dispatch failures and continue with the sale confirmation flow.
            }
    ...
    private function buildItems(array $input): array
    {
        ...
        foreach ($descriptions as $index => $description) {
            ...
            $items[] = [
                ...
                'type' => $iccidId !== null ? 'sim' : 'service',
            ];
        }

        $productIds = $input['product_id'] ?? [];
        $productDescriptions = $input['product_description'] ?? [];
    ...
    public function loadSaleForRefund(int $saleId): array
    {
        ...
        return [
            ...
            'sale' => [
                ...
                'customer_id' => isset($sale['customer_id']) ? (int) $sale['customer_id'] : null,
                'customer_fullname' => $sale['customer_fullname'] ?? null,
                'customer_email' => $sale['customer_email'] ?? null,
                'customer_phone' => $sale['customer_phone'] ?? null,
                'customer_tax_code' => $sale['customer_tax_code'] ?? null,
                'customer_note' => $sale['customer_note'] ?? null,
