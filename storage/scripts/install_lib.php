    ...
    final class AppInstaller
    {
        ...
        public static function run(array $dbConfig, array $options): array
        {
            ...
            try {
                ...
                $pdo->exec($sql);
                $messages[] = 'Schema importato correttamente.';
        ...
        public static function runUpgrade(array $dbConfig): array
        {
            ...
            try {
                ...
                if ($migrations === []) {
                    $messages[] = 'Nessun file di migrazione trovato.';
                }
                ...
                foreach ($migrations as $file) {
                    ...
                    if ($checksum === false) {
                        throw new \RuntimeException('Impossibile calcolare il checksum per ' . $basename);
                    }
                    ...
                    if ($row !== false) {
                        if ($row['checksum'] !== $checksum) {
                            $messages[] = "Migrazione '$basename' già applicata ma il file è stato modificato (checksum differente).";
                        } else {
                            $messages[] = "Migrazione '$basename' già applicata.";
                        }
                        continue;
                    }
                    ...
                    if ($sql === false) {
                        throw new \RuntimeException('Impossibile leggere il file di migrazione ' . $basename);
                    }
                    ...
                    $messages[] = "Migrazione '$basename' applicata.";
                    $applied++;
                }

                if ($applied === 0 && $migrations !== []) {
